...
object ispname {
  def main(args: Array[String]): Unit = {
    ...
    df.map(row =>{
      ...
      (ispname,requestlist++clicklist++Adlist)
    }).reduceByKey((list1,list2)=>{
      list1.zip(list2).map(x=>x._1 + x._2)
    }).map(x=>{
      x._1 +","+ x._2.mkString(",")
    })
      .saveAsTextFile("G:/outputPath/output-20190822-2")



//    val frame = sqlContext.sql("select ispname," +
...
//      "max(case when iseffective =1 and isbilling =1 and iswin =1 then 1 else 0 end) t9 from tmp group by ispname")
//
//      frame.show()
