...
object networkmannername {
  def main(args: Array[String]): Unit = {
    ...
    df.map(row => {
      ...
      x._1 +","+ x._2.mkString(",")
    }).saveAsTextFile("G:/outputPath/output20190822-3")

//    val frame = sqlContext.sql("select networkmannername," +
...
//      "max(case when iseffective =1 and isbilling =1 and iswin =1 then 1 else 0 end) t9 from tmp group by networkmannername order by networkmannername")
//
//    frame.show()
