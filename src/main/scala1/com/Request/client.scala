...
object client {
  def main(args: Array[String]): Unit = {
    ...
    df.map(row =>{
      ...
      (RptUtils.androidAndios(client),requestlist++clicklist++Adlist)
      ...
      .saveAsTextFile("G:/outputPath/output-20190822-5")


//    val frame = sqlContext.sql("select client," +
...
//      "max(case when iseffective =1 and isbilling =1 and iswin =1 then 1 else 0 end) t9 from tmp group by client")
//
//    frame.show()
  }
}
